...
export default function LogGrocery() {
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      ...
      const res = await analyzeImage(base64, 'grocery', description || undefined)
  ...
  const handleTextSubmit = async () => {
    ...
    try {
      const res = await analyzeImage(null, 'grocery', description)
      setAnalysis(res.data)
      setStep(STEPS.RESULT)
    } catch (err) {
      setError('Analisi non riuscita. Riprova.')
  ...
  return (
    <div className="pb-4 px-4">
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-on-surface">Registra Spesa</h2>
      </div>
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
            ...
            <label className="flex flex-col items-center gap-3 p-6 border-2 border-dashed border-outline-variant/40 rounded-2xl cursor-pointer hover:bg-surface-container-high/30 transition-colors">
              ...
              <span className="text-sm text-on-surface/60">Fotografa lo scontrino o i prodotti</span>
              <input
                type="file"
                accept="image/*"
                capture="environment"
          ...
          {/* Text description */}
          <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
            <p className="font-bold text-on-surface mb-3">Descrivi la spesa</p>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Es: Frutta e verdura di stagione, latte locale, pane dal forno"
        ...
        <div className="mt-6 space-y-4">
          {imagePreview && (
            <div className="rounded-2xl overflow-hidden shadow-card">
              <img src={imagePreview} alt="Foto della spesa" className="w-full h-48 object-cover" />
            </div>
          )}
          ...
          {analysis.result?.items?.length > 0 && (
            <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
              <p className="font-bold text-on-surface mb-2">Prodotti rilevati</p>
