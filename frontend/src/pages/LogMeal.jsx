...
export default function LogMeal() {
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      ...
      const res = await analyzeImage(base64, 'meal', description || undefined)
  ...
  const handleTextSubmit = async () => {
    ...
    try {
      const res = await analyzeImage(null, 'meal', description)
      setAnalysis(res.data)
      setStep(STEPS.RESULT)
    } catch (err) {
      setError('Analisi non riuscita. Riprova.')
  ...
  return (
    <div className="pb-4 px-4">
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-on-surface">Registra Pasto</h2>
      </div>
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
            ...
            <label className="flex flex-col items-center gap-3 p-6 border-2 border-dashed border-outline-variant/40 rounded-2xl cursor-pointer hover:bg-surface-container-high/30 transition-colors">
              ...
              <span className="text-sm text-on-surface/60">Tocca per fotografare il pasto</span>
              <input
                type="file"
                accept="image/*"
                capture="environment"
          ...
          {/* Text description */}
          <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
            <p className="font-bold text-on-surface mb-3">Descrivi il pasto</p>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Es: Pasta al pomodoro con insalata mista e pane integrale"
      ...
      {step === STEPS.RESULT && analysis && (
        <div className="mt-6 space-y-4">
          ...
          {analysis.result?.items?.length > 0 && (
            <div className="bg-surface-container-lowest rounded-2xl shadow-card p-5">
              <p className="font-bold text-on-surface mb-2">Elementi rilevati</p>
